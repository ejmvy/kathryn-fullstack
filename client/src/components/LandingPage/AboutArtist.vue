<template>
  <section class="section py-14">
    ...
    <div class="flex flex-col md:flex-row w-4/5 lg:w-2/3 py-20 items-center">
      ...
      >
        <h1 class="title1 text-2xl pb-10">{{ artistName }}</h1>
        <p class="text-sm leading-8 text-gray-500">{{ artistBlurb }}</p>
      </div>
      <img
        class="order-1 w-80 h-80 sm:w-96 sm:h-96 rounded-full"
        src="../../assets/latestDesigns/artist.jpg"
      />
    </div>
  </section>
</template>
...
export default {
  ...
  setup() {
    const titleHeader = "From Lockdown to Passion";
    const artistName = "Kathryn O'Kelly";
    const artistBlurb =
